...
### 0.8.2 — איתור ההודעות והצגת גרסה

בראש החלון מוצגת גרסת היישום; אם הרכיבים המותקנים שונים, מופיעה הנחיה לעדכנם. איתור אזור הגלילה בודק את אבות כל ההודעות הגלויות, מתעלם מכותרות וציטוטים וממתין לטעינה. זיהוי עמום עדיין עוצר העלאה. דוח הבדיקה כולל מידות ומאפייני גלילה ללא תוכן ההודעות, וקוד התקלה המקורי נשמר.

בדוח של מיכל מ־24.9 בשעה 18:51 מותקן 0.8.1 ונרשם ניסיון בדיקת התאמה שנכשל לפני קריאת קבצים; לא נרשם ניסיון שחזור. יש להחליף את היישום, ללחוץ על ״עדכון רכיבי הסוכן״, ואז על ״בדיקת הפעולה הקודמת״. רק לאחר סיום האימות ממשיכים לבדיקת התאמה, עדכון עכשיו והפעלת התזמון. אין למחוק מצב ממתין או להעלות שוב קבצים כדי לעקוף את החסימה. בדיקות מקומיות אינן אישור שהפעולה הממתינה במחשב של מיכל כבר שוחזרה.


### 0.9.0 — מסלול הקמה ואבחון סריקה

שלושה שלבים: חיבור, בדיקת קבצים, הפעלה. הכותרת והפעולה הראשית נשארות קבועות והתוצאות נגללות ביניהן. כלי תחזוקה, חיבור מחדש, דיווחים, תיקייה ותזמון נמצאים בחלון הגדרות נפרד. פעולה קודמת מוצגת רק כשנותרה פעולה לא מאומתת; דיווח שממתין לאישור אינו חוסם בדיקות. בעת שגיאה מוצג יצוא דוח ליד ההסבר.

בסריקה לא מלאה מבוצע ניסיון קריאה נוסף אחד בדפדפן חדש, ללא ניסיון חוזר של כתיבה. מזהי הודעות בתוך ציטוטים או כותרות אינם נחשבים להודעות עצמאיות. אם הודעה מוכרת עדיין חסרה, נשמרים שמה, מועד הקבלה ומזהה המקור בדוח. ההעלאה עדיין חסומה: אין למחוק מטמון או להחזיר קבצים שלא נצפו כדי לעקוף זאת. בצילומי 24.9 בערב הפעולה הקודמת כבר אומתה; חסר דוח חדש לקביעת הסיבה המדויקת להודעה החסרה.

בדיקת מסלול מקומית ללא XCTest: קומפלו באמצעות swiftc את AgentServices.swift, AgentWorkflow.swift ואת test/native-workflow.swift והריצו את הקובץ שנוצר. בדיקות אלה אינן מפעילות חיבור אמיתי.

### 0.9.1 — מחזור חדש ומניעת שינה
- לאחר השלמת כל החוברות ידנית: בהגדרות בחרו ״התחלת מחזור חדש אחרי עדכון ידני״ ואשרו. זו הצהרת מפעיל, לא הוכחה שהקבצים בשרת נכונים. הפעולה נשמרת ביומן ואינה מוחקת קבצים, החלטות או אישורי העלאה.
- הסוכן סורק ללא העלאה רק מהנקודה החדשה. אחרי סריקה מלאה וסנכרון מוצלח אפשר להפעיל גם אם עדיין אין קבצים חדשים. הודעות חסרות מלפני הנקודה לא חוסמות; הודעות חסרות חדשות עדיין מחייבות בדיקה. לא מדלגים על פעולה קודמת שהעלאתה לא אומתה.
- נקודת ההתחלה חלה גם על השלמת מועדי עבר: לא מריצים שוב פעימות קודמות. המועדים הבאים נשמרים, לרבות ראשון 27.9 בשעות שהוגדרו באתר. שינוי חשבון או קבוצה מחייב קביעת מחזור מתאים מחדש.
- הפעלת האוטומציה יוצרת LaunchAgent פר משתמש עם caffeinate -i ומוודאת assertion של התהליך הנכון מול pmset. עצירה מסירה את רכיב מניעת השינה. מצב מניעת השינה מוצג בחלון ובדוח; המסך יכול לכבות ולהינעל. לאחר אתחול נדרשת כניסה לחשבון המק. שינה ידנית, כיבוי והפסקת חשמל אינם מכוסים.
- אימות: בדיקות אוטומטיות, בנייה לשתי ארכיטקטורות ובדיקת מניעת שינה מבודדת במק. הפעלה ובדיקה מתוזמנת במק של מיכל עדיין נדרשות; הבדיקות לא העלו PDF ולא שלחו הודעות לקבוצה.

### 2026-09-25 — הודעות מייל ברורות (שרת 1.20.21)
- מיילי מצב מסוכן וואטסאפ מציגים שם מחשב מהרישום בשרת, מועד בשעון ישראל, תוצאה ופעולה נדרשת. `no_files` הוא דיווח רגיל, לא תקלה או הודעת התקנה. `ready` מעיד על בדיקה מוצלחת ואינו מבטיח העלאת קבצים.
- דוחות עדכון מציגים סיכום קצר ולאחריו רק סעיפים עם תוכן. נשמרים שם החוויה, שם הקובץ המדויק וזמן ההחלפה לכל קובץ; כפילויות שומרות את המועד המקורי. מזהי ריצה/הודעות והסברים טכניים נשארים ביומן, ולא נשלחים במייל. חוסרים משמעם שלא התקבל קובץ מתאים בסריקה, ולא שהחוברת באתר בהכרח לא מעודכנת.
- שם המחשב נשמר בריצה מתוך חיבור מאומת ובדיקת בעלות; דוח חוזר משתמש בשם שנשמר. לדוחות ישנים ללא שם מוצג שהשם לא תועד, בלי לנחש שמדובר במחשב האחרון שהיה פעיל.
- אין שינוי בנמען, במניעת הודעות כפולות או במנגנון העלאת הקבצים. השינוי חל על מיילים עתידיים ללא התקנת סוכן חדש. בדיקות מקומיות משתמשות בספק מייל מדומה; לא נשלחו הודעות בדיקה. קריאת דואר עבר דרך מפתח Resend הקיים אינה זמינה (הרשאת שליחה בלבד); הבדיקה התבססה על הצילום ועל כל תבניות הסוכן בקוד.
