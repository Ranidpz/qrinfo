...
- Raffle animation styles: `RaffleStage.tsx` picks between the classic wheel (`RaffleDisplay`) and code-reveal (`RaffleCodeReveal`) off `RaffleConfig.animationStyle` — absent means wheel, so old raffles are untouched. Both take identical props. Code-reveal is for lists of CODES (the code sits in `firstName`, so `participantLabel` returns it — no data-model change): characters scramble out of real per-position pools sampled from the loaded codes, then lock left→right with a pace that slows on the last two. Its draw fires on the FIRST frame of the click (the wheel draws ~2.5s in), so network latency only lengthens the scramble instead of freezing the screen; a failed draw returns to idle and is NEVER auto-retried (a lost response after a committed transaction would burn a second winner). Pools snapshot at run start so the mid-run `fetchNames()` can't disturb the screen. Tick/pop sounds are synthesized in `src/lib/raffle/codeAudio.ts` (Web Audio, not files — blips every ~70ms can't retrigger through `<audio>`), gated by `codeTickSounds` under the `soundsEnabled` master. rAF stops dead in a hidden tab, so a `visibilitychange` handler shifts the lock deadline by exactly the time away — never key that off frame-gap size, which defers forever whenever frames are merely slow.
- Raffle participant import (v1.19.2): ONE parser in `src/lib/raffle/import.ts` (`parseMatrix` for the Excel drop, `parseText` for the paste box) feeds a mandatory preview in `RaffleImportPanel.tsx` — nothing is written straight from a file. `RaffleConfig.listType` ('people' | 'codes', absent = people) drives the table columns, the downloaded template and the dedup key. The bug it replaced: the old importer only recognised name/phone headers, so a code file fell through to positional columns — the serial column landed in `firstName`, the code in `lastName`, and the header row itself became a participant (3,000 codes → 3,001 rows). Header detection now spans code/serial keywords too, and in codes mode the column is chosen from the DATA (widest consistently-filled column that isn't a running serial), never by position. Server doc id (`docIdFor` in the participants route) is the code in codes mode — that's what makes a code list dedupe at all — and in people mode a no-phone row on a MERGE gets a random id, since the old `row-${i}` would overwrite an unrelated existing row. Bulk delete posts ids in the DELETE body (a URL would blow up on thousands). The table renders at most `RENDER_LIMIT` 200 rows while select-all/delete still act on the whole filtered set — 3,000 rows in the DOM made every checkbox click re-render everything. Parser is covered by a standalone suite (scratchpad, 22 assertions) run against the real broken file shape.
